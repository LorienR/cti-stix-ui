...
/**
 * Report New Controller responsible for creating records
 *
 * @module
...
export default Ember.Controller.extend({
    ...
    saveItem(item) {
        ...
        const self = this;
        const store = this.get("store");
        let object_refs = this.get("model.item.object_refs");

        
        intrusionSets.forEach(function(intrusionSet) {
            let invalid = true;
            if (Ember.isPresent(intrusionSet.id)) {
                    invalid = false;
            }
            if (invalid) {
                intrusionSets.removeObject(intrusionSet);
            }
            else{
                object_refs.push(intrusionSet.id);
            }
        });
        attackPatterns.forEach(function(attackPattern) {
            let invalid = true;
            if (Ember.isPresent(attackPattern.id)) {
                    invalid = false;
            }
            if (invalid) {
                intrusionSets.removeObject(attackPattern);
            }
            else{
                object_refs.push(attackPattern.id);
            }
        });
